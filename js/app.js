...
class App {
    ...
    _stickyNavbar() {
        ...
        const navbar = new IntersectionObserver(this._navbarSticky, {
            root: null,
            threshold: 0,
    ...
    _activeLink() {
        links.forEach((link) =>
            link.addEventListener('click', function (e) {
                const link = e.target;
                const sibilings = link
                    .closest('.links')
                    .querySelectorAll('.link');
                sibilings.forEach((sibiling) => {
                    if (sibiling === link)
                        sibiling.style.color = 'rgb(20, 184, 166)';
                    else sibiling.style.color = ' rgb(209, 213, 219)';
                });
            })
        );
    }
